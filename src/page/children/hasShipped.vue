...
<script type="text/javascript">
    ...
    import { List,SearchDate } from '@/service/getData';
	export default{
        props:{
            fileType : {
                type : String,
		...
		methods : {
            ...
            upCallback: function(page) {
                //联网加载数据
                var self = this;
                console.log(this.searchNeedDate.type);
                if(this.searchValue == '' && this.searchNeedDate.type == ""){
                    ...
                    this.searchNeedDate.content = this.searchValue;
                    this.searchNeedDate.type = this.SelectType;
                    this.searchNeedDate.fileType = this.fileType;
                    console.log(JSON.stringify(this.searchNeedDate));
                   getSearchDataFromNet(this.searchNeedDate,page.num, page.size, function(curPageData) {
                        //curPageData=[]; //打开本行注释,可演示列表无任何数据empty的配置                       
                        //如果是第一页需手动制空列表 (代替clearId和clearEmptyId的配置)
                        if(page.num == 1) self.listName = [];
                        //更新列表数据
...
function getSearchDataFromNet(searchDate,pageNum,pageSize,successCallback,errorCallback) {
    setTimeout(function(){
        SearchDate().then(res => {
            var data=res.data;
            var listData=[];
